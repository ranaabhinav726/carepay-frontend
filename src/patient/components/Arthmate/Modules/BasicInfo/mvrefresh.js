...
const RazorpayRefresh = () => {
    ...
    const [leadStatus, setStatusData] = useState('');

    let navigate = useNavigate()
    useEffect(() => {
        let userId = (window.location.href).split('/patient/mvrefresh/')[1]
    ...
    return (
        <main className='congrats'>
            ...
            <p className='text-center'>fetching income verification status...</p>
            ...
            {leadStatus === 'DOCS_REQUIRED' ?
                <button
                    onClick={() => uploadAgain()}
                    className=''
                    style={{ marginTop: '20px', padding: '15px', color: '#ecebfd', background: '#504c9a', border: 'none', borderRadius: '5px', fontSize: '14px', fontWeight: '700', width: '100%' }}
                >
                    Upload Statement Again
                </button>
                : ""}
